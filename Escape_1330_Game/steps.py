...
def movePlayer(mapList, playerMove, playerPos, gamemode, numOfRound, currentValue = None, currentPlayerPos = None):
    ...
    if gamemode == 4:
        ...
        if 'boom' in currentValue:
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    if not (i == 0 and j == 0) and (currentPlayerPos[0]+i) in range(7) and (currentPlayerPos[0]+j)in range(16) and mapList[currentPlayerPos[0]+i][currentPlayerPos[1]+j] not in noDigitList:
